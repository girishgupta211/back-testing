...
if common_n200_mid:
    ...
    years_span = days_span / 365.25
    print(f'  Duration: {days_span} days ({years_span:.2f} years)')
    print(f'  Extra days vs 3-index: {len(common_n200_mid) - len(common_all)}')
...
print(f'\n📅 NIFTY200 MOMENTUM:')
print(f'  Starts: {min(nifty200_dates).date()}')
print(f'  Ends: {max(nifty200_dates).date()}')
print(f'  Total: {len(nifty200_dates)} trading days')

print(f'\n📅 MIDCAP150 M50:')
print(f'  Starts: {min(midcap_dates).date()}')
print(f'  Ends: {max(midcap_dates).date()}')
print(f'  Total: {len(midcap_dates)} trading days')
print(f'  ⚠️  Missing before: {min(midcap_dates).date()}')

print(f'\n📅 SMALLCAP250 MQ100:')
print(f'  Starts: {min(smallcap_dates).date()}')
print(f'  Ends: {max(smallcap_dates).date()}')
print(f'  Total: {len(smallcap_dates)} trading days')
print(f'  ⚠️  Missing before: {min(smallcap_dates).date()}')

print('\n' + '='*80)
print('BACKTESTING OPTIONS')
print('='*80)

print(f'\n🎯 OPTION 1: Full Portfolio (all 3 indices)')
print(f'   Period: {min(common_all).date()} to {max(common_all).date()}')
print(f'   Duration: {(max(common_all) - min(common_all)).days} days ({((max(common_all) - min(common_all)).days/365.25):.2f} years)')
print(f'   Trading days: {len(common_all)}')
print(f'   Status: ⚠️  Only ~1 year (need 2 years)')

print(f'\n🎯 OPTION 2: Partial Portfolio (Nifty200 + Midcap only)')
print(f'   Period: {min(common_n200_mid).date()} to {max(common_n200_mid).date()}')
print(f'   Duration: {(max(common_n200_mid) - min(common_n200_mid)).days} days ({((max(common_n200_mid) - min(common_n200_mid)).days/365.25):.2f} years)')
print(f'   Trading days: {len(common_n200_mid)}')
print(f'   Allocation: Nifty200 33% | Midcap 67% (adjusted without Smallcap)')
print(f'   Status: ✅ Almost 2 years!')
